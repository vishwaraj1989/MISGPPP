    ...
    const fetchData = async (userId) => {
      try {
        ...
        const filteredRows = response.data.filter(row => 
          row.srStatus === "OPEN" && row.category === "Residential" && row.srType === "New Connection LT" && (row.fqMrDate !== null && row.fqMrDate !== undefined && row.fqMrDate.trim() !== "")
        );
...
export default function RgprPP() {
  return (
    <Box sx={{ display: 'flex' }}>
      <Sidenav />
      <Box sx={{ flexGrow: 1 }}>
