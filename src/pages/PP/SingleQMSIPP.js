...
function EnhancedTable() {
    ...
    axios.get('/api/formRoutes') // Adjust the API endpoint as needed
      .then(response => {
        const filteredRows = response.data.filter(row => 
          row.srStatus === "OPEN" && row.phase === "Single Phase" && row.category === "Manufacturing & Service Industries" && row.srType === "New Connection LT" && (row.fqMrDate !== null && row.fqMrDate !== undefined && row.fqMrDate.trim() !== "")
        );
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, 'SingleQMSIPP.xlsx');
  };
...
export default function SingleQMSIPP() {
  return (
    <Box sx={{ display: 'flex' }}>
      <Sidenav />
      <Navbar />
