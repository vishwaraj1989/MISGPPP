    ...
    axios.get('/api/formRoutes') // Adjust the API endpoint as needed
      .then(response => {
        const filteredRows = response.data.filter(row => 
          row.srStatus === "OPEN" && row.phase === "Single Phase" && row.category === "Manufacturing & Service Industries" && row.srType === "Change of Load for LT Reduction" && (row.fqMrDate !== null && row.fqMrDate !== undefined && row.fqMrDate.trim() !== "")
        );
