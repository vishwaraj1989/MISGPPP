...
function EnhancedTable() {
    ...
    axios.get('/api/formRoutes') // Adjust the API endpoint as needed
      .then(response => {
        const filteredRows = response.data.filter(row => 
          row.srStatus === "OPEN" && row.phase === "Single Phase" && row.category === "Manufacturing & Service Industries" && row.srType === "Change of Load for LT Addition" && (row.fqMrDate !== null && row.fqMrDate !== undefined && row.fqMrDate.trim() !== "")
        );
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, 'SingleQMSILePP.xlsx');
  };
          ...
          >
            ...
            <TableBody>
              ...
              {emptyRows > 0 && (
                <TableRow
                  style={{
                    height: 53 * emptyRows, // Adjusted for 'medium' size
                  }}
                >
...
export default function SingleQMSILePP() {
  return (
    <Box sx={{ display: 'flex' }}>
      <Sidenav />
      <Navbar />
