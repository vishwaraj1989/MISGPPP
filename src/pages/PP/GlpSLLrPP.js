...
function EnhancedTable() {
    ...
    axios.get('/api/formRoutes') // Adjust the API endpoint as needed
      .then(response => {
        const filteredRows = response.data.filter(row => 
          row.srStatus === "OPEN" && (row.category === "General Lighting Purpose" || row.category === "Street Light") && row.srType === "Change of Load for LT Reduction" && (row.fqMrDate !== null && row.fqMrDate !== undefined && row.fqMrDate.trim() !== "")
        );
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, "GlpSLLrPP.xlsx");
  };
...
export default function GlpSLLrPP() {
  return (
    <Box sx={{ display: 'flex' }}>
      <Sidenav />
      <Box sx={{ flexGrow: 1 }}>
