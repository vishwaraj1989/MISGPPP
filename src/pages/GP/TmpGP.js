...
function EnhancedTable() {
  ...
  useEffect(() => {
    ...
    const fetchData = async (userId) => {
      try {
        ...
        const filteredRows = response.data.filter(row => row.srStatus === "OPEN" && row.category === "Temporary" && row.srType === "New Connection LT" && (!row.fqMrDate || row.fqMrDate.trim() === ""));
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(workbook, 'registration_entries.xlsx');
  };
