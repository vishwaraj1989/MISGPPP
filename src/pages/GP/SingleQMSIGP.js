...
function EnhancedTable() {
  ...
  useEffect(() => {
    ...
    fetchUserData().then(fetchData); // Chain fetchData after fetchUserData
  }, []);
  ...
  const sortedRows = [...rows].sort((a, b) => {
    const aValue = orderBy === 'dateOfSurvey' || orderBy === 'rcDate' ? parseDate(a[orderBy]) : a[orderBy];
    const bValue = orderBy === 'dateOfSurvey' || orderBy === 'rcDate' ? parseDate(b[orderBy]) : b[orderBy];
...
export default function SingleQMSIGP() {
  return (
    <Box sx={{ display: 'flex' }}>
      <Sidenav />
      <Navbar />
