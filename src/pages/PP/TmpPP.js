...
//   { id: 'trAmount', numeric: false, disablePadding: false, label: 'TR Date' },
...
//   const { numSelected, Export } = props;
...
//           row.srStatus === "OPEN" && row.category === "Temporary" && (row.fqMrDate !== null && row.fqMrDate !== undefined && row.fqMrDate.trim() !== "")
//         );
...
//         formattedRow[cell.label] = row[cell.id];
//       });
//       return formattedRow;
//     }));
//     const workbook = XLSX.utils.book_new();
//     XLSX.utils.book_append_sheet(workbook, worksheet, 'Sheet1');
//     XLSX.writeFile(workbook, 'TmpPP.xlsx');
//   };
...
//         <EnhancedTableToolbar numSelected={0} Export={Export} />
//         <TableContainer>
//           <Table
//             sx={{ minWidth: 750 }}
...
//                       <TableCell align="center">{row.fqDate}</TableCell>
...
function EnhancedTable() {
  const [order, setOrder] = useState('asc');
  const [orderBy, setOrderBy] = useState('surveyCategory');
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    axios.get('/api/formRoutes') // Adjust the API endpoint as needed
      .then(response => {
        const filteredRows = response.data.filter(row => 
          row.srStatus === "OPEN" && row.category === "Temporary" && row.srType === "New Connection LT" && (row.fqMrDate !== null && row.fqMrDate !== undefined && row.fqMrDate.trim() !== "")
        );
        setRows(filteredRows);
        setLoading(false);
      })
  ...
  const Export = () => {
    const worksheet = XLSX.utils.json_to_sheet(rows.map(row => {
      const formattedRow = {};
      headCells.forEach(cell => {
        formattedRow[cell.label] = row[cell.id] !== null ? row[cell.id] : 'N/A';
      });
      return formattedRow;
    }));
    const workbook = XLSX.utils.book_new();
